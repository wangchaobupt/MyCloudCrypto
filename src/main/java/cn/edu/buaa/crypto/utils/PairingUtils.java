...
//    public static final String PATH_a1_2_512 = "params/a1_2_512.properties";
    ...
    public static byte[] Xor(byte[] ele1, byte[] ele2) {
        ...
        byte res[] = new byte[longbytes.length];
        int i = 0;
        for (; i < shortbytes.length; i++) {
            res[i] = (byte)(shortbytes[i]^longbytes[i]);
        }
...
//    }
    ...
    //伪随机函数
//    public static Element pseudoFunc(Pairing pairing, String key, int id) {
//        try {
...
//    }
    ...
    public static byte[] PF(Pairing pairing, byte[] key, byte[] id){
        byte[] m = new byte[key.length + id.length];
        System.arraycopy(key,0,m,0,key.length);
        System.arraycopy(id,0,m,key.length,id.length);
        return hash(m);
    }
