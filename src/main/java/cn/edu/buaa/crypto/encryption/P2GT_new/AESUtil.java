...
//            //生成一个128位的随机源,根据传入的字节数组
            // FGTODO:2019-10-14 使用下面这两行方法，因为SecureRandom是根据系统进行执行的，所以linux和window不太一样，会导致解密失败
//            keygen.init(128, new SecureRandom(encodeRules.getBytes()));

            // FGTODO:2019-10-14 这种方式，亲测有效
            KeyGenerator keygen = KeyGenerator.getInstance("AES");
...
//            keygen.init(128, new SecureRandom(encodeRules.getBytes()));
     ...
     * @param secureRandom
     */
    public static byte[] initKey(SecureRandom secureRandom) throws Exception{
        //密钥生成器
        KeyGenerator keyGen = KeyGenerator.getInstance("AES");
        //初始化密钥生成器
