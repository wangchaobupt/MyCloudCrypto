...
import cn.edu.buaa.crypto.encryption.ASFlow.SAACE_RW.*;
import cn.edu.buaa.crypto.encryption.ASFlow.SACABE.MasterPublicKey;
import cn.edu.buaa.crypto.encryption.ASFlow.SACABE.MasterSecretKey;
import cn.edu.buaa.crypto.encryption.ASFlow.SACABE.SACABEEngine;
...
public class SAACE_ACEngine {
    private static SAACE_ACEngine engine;
    private Pairing pairing;
    private SACABEEngine engine_ace = new SACABEEngine();
    ...
    public static SAACE_ACEngine getInstance() {
        if (engine == null) {
            engine = new SAACE_ACEngine();
        }
        return engine;
    }
    ...
    public MasterKey Setup(String perperties, String[] U, int max) {
        ...
        svk = sps_allKey.svk;
        crs = new CRS(mpk.g, mpk.h, mpk.h1, mpk.h2);
    ...
    public EncryptionKey EKGen(MasterKey mk, String[] T, int t, String[] A) throws NoSuchAlgorithmException {
        SignKey ak = engine_abs.KeyGen(mk.ask, T, t);
        int k = A.length;
        Element[] V = new Element[k];
        for (int i = 0; i < k; i++) {
            String input_for_hash = A[i] + 1 + 1;
    ...
    public cn.edu.buaa.crypto.encryption.ASFlow.SACABE.SecretKey DKGen(MasterKey mk, String[] R) throws NoSuchAlgorithmException {
    ...
    public CipherText Encrypt(SignParameter sm, EncryptionKey ek) throws NoSuchAlgorithmException {
        String[] A = ek.P.A;
        String accessPolicy = "";
        ...
        cn.edu.buaa.crypto.encryption.ASFlow.SACABE.CipherText c = engine_ace.Encrypt(sm.m, accessPolicy);
    ...
    public CipherTextParameter Encrypt_NIZK(CipherText CT, Statement statement, SignParameter sm, EncryptionKey ek) {
        Witness witness = new Witness(sm.m, engine_ace.getS1(), engine_ace.getS2(), ek.sign, sm.sign);
    ...
    public Statement getStatement(CipherText CT) {
        Statement statement = new Statement(evk, svk, CT.ct_ac, CT.P.T, CT.B, CT.P.A, CT.Va, CT.Vb);
    ...
    public Proof NIZK_Prove(Statement statement, Witness witness) {
        ...
        Element A1 = pairing.pairing(g, h1).powZn(r1).getImmutable();
        Element A2 = pairing.pairing(g, h2).powZn(r2).getImmutable();

        Element x2 = pairing.getZr().newRandomElement().getImmutable();
        Element B = mpk.T1.powZn(x2.add(r1)).mul(mpk.T2.powZn(x2.add(r2))).getImmutable();
        ...
        CHashParameter Cbytes = new CHashParameter(statement.ct_ac.C, statement.ct_ac.ct0.get(0), statement.ct_ac.ct0.get(1),
                Q, P, R, A1, A2, B, E1, E2, E3, E4, E5, E6);
        Element c = PairingUtils.MapByteArrayToGroup(pairing, Cbytes.getCbytes(), PairingUtils.PairingGroupType.Zr);

        Element y1 = r1.sub(c.mul(witness.s1)).getImmutable();
        Element y2 = r2.sub(c.mul(witness.s2)).getImmutable();
        Element n = mpk.T1.powZn(x2).mul(mpk.T2.powZn(x2)).mul(witness.m.powZn(c.negate())).getImmutable();
    ...
    public boolean NIZK_Verify(Statement statement, Proof proof) {
        ...
        Element A1 = pairing.pairing(g, h1.powZn(proof.y1).mul(statement.ct_ac.ct0.get(0).powZn(c))).getImmutable();
        Element A2 = pairing.pairing(g, h2.powZn(proof.y2).mul(statement.ct_ac.ct0.get(1).powZn(c))).getImmutable();

        Element B = proof.n.mul(mpk.T1.powZn(proof.y1)).mul(mpk.T2.powZn(proof.y2)).mul(statement.ct_ac.C.powZn(proof.c)).getImmutable();
    ...
    public cn.edu.buaa.crypto.encryption.ASFlow.SACABE.CipherText Sanitize(CipherTextParameter CTparameter, Statement statement) throws Exception {
        CipherText CT = CTparameter.CT;
        cn.edu.buaa.crypto.encryption.ASFlow.SACABE.CipherText c = CT.ct_ac;
        cn.edu.buaa.crypto.encryption.ASFlow.SACABE.CipherText ct;
    ...
    public Element Decrypt(cn.edu.buaa.crypto.encryption.ASFlow.SACABE.CipherText ct, cn.edu.buaa.crypto.encryption.ASFlow.SACABE.SecretKey sk) {
        return engine_ace.Decrypt(ct, sk);
    }
}
