...
public abstract class CPABEEngine extends Engine {
    ...
    public PairingCipherSerParameter encryption(PairingKeySerParameter publicKey, String accessPolicy, Element message) throws PolicySyntaxException {
        ...
        for(int i=0;i<accessPolicyIntArrays.length;i++){
            for(int j=0;j<accessPolicyIntArrays[i].length;j++){
                System.out.print(accessPolicyIntArrays[i][j]+" ");
            }
            System.out.println();
        }
        System.out.println("rhos:");
        for(int i=0;i<rhos.length;i++)
            System.out.print(rhos[i]+" ");
        System.out.println();
